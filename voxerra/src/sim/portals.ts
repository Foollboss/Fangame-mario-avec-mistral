 ...
 * de braise (Surface ↔ Abîme, échelle 1:4), autels astraux (Surface ↔ Cimes),
 * construction des portails d'arrivée et liens mémorisés dans la sauvegarde.
 */
...
export function tryIgnitePortal(sim: Sim, w: World, x: number, y: number, z: number): boolean {
  const t = w.content.blocks;
  const frame = t.tryNum('pierre_runique');
  const portal = t.tryNum('voile_abime');
...
export function checkPortalContact(sim: Sim, p: Player, dt: number): void {
  ...
  if (p.portalTime >= (p.creative ? 0.5 : 2.5)) {
    p.portalTime = 0;
    p.portalCooldown = 6;
    const to: DimensionId = id === astral ? (p.dim === 'astral' ? 'surface' : 'astral') : p.dim === 'abime' ? 'surface' : 'abime';
    sim.requestTravel(p, to, { x: Math.floor(p.x), y: Math.floor(p.y), z: Math.floor(p.z) });
  }
}
 ...
 */
export function buildArrivalPortal(sim: Sim, dim: string, x: number, z: number, preferY: number): { x: number; y: number; z: number } {
  ...
  if (best === null) {
    ...
    for (let a = -3; a <= 4; a++) for (let k = -1; k <= 5; k++) for (let b = -2; b <= 2; b++) w.setBlock(x + a, y + k, z + b, k === -1 ? t.tryNum(dim === 'abime' ? 'basalte_poli' : 'pierre_taillee') : 0, { silent: true });
  }
  ...
  for (let a = -1; a <= 2; a++)
    for (const b of [-1, 1]) {
      if (!t.solid[w.getId(x + a, y - 1, z + b)]) w.setBlock(x + a, y - 1, z + b, t.tryNum(dim === 'abime' ? 'basalte_poli' : 'pierre_taillee'), { silent: true });
