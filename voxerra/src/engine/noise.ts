...
/**
 * Bruit Simplex 2D/3D (algorithme de Stefan Gustavson, domaine public),
 * avec une table de permutation dérivée de la graine.
 */
...
export class SimplexNoise {
  ...
  /** Mouvement brownien fractal 2D, normalisé dans ~[-1,1]. */
  ...
  fbm3(x: number, y: number, z: number, octaves: number, lacunarity = 2, gain = 0.5): number {
    let sum = 0,
      ...
      norm = 0;
    for (let o = 0; o < octaves; o++) {
      sum += amp * this.noise3(x * freq + o * 19.1, y * freq, z * freq - o * 23.9);
      norm += amp;
      amp *= gain;
      freq *= lacunarity;
    }
  ...
  /** Bruit « crêtes » : 1 au sommet des crêtes, 0 ailleurs. */
  ridged2(x: number, y: number, octaves: number): number {
    let sum = 0,
      ...
      norm = 0;
    for (let o = 0; o < octaves; o++) {
      const n = 1 - Math.abs(this.noise2(x * freq + o * 13.1, y * freq + o * 7.7));
      sum += amp * n * n;
      norm += amp;
      amp *= 0.5;
      freq *= 2;
